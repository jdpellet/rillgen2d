...
def function_decorator(func):
    def wrapper(*args, **kwargs):
        try:
            ...
            # If the exception occurs in a class method, we want to log the error to the console then re-raise it
            print(e)
            if type(args[0]) is Rillgen2d:
                # Sometimes there is an error that console isn't an attribute of the class
                rillgen = args[0]
                if "console" in dir(rillgen):
                    args[0].console.put(e)
                if getattr(rillgen, "threads"):
                    for thread in rillgen.threads:
...
def public_function_decorator(func):
    @function_decorator
    def wrapper(*args, **kwargs):
        try:
            return func(*args, **kwargs)
...
class Rillgen2d(Thread):
    ...
    def setup_rillgen(self):
        ...
        self.run_command("gcc -Wall -shared -fPIC ../rillgen2d.c -o ./rillgen.so")
        self.rillgen = CDLL( Path.cwd() / 'rillgen.so') 
