...
long t,count,*topovecind,*flatis,*flatjs,*iup,*idown,*jup,*jdown,lattice_size_x,lattice_size_y,ic,jc;
int **i1,**jl,**i2,**j2,flagformode,flagforroutingmethod,numberofraindata,numberofraingages;
...
{    float rmax,smax,s1,s2,r,s,diff;
     ...
     r=rmax;
	 if ((mask[i][j]==1)&&(i>1)&&(i<lattice_size_x)&&(j>1)&&(j<lattice_size_y))
        ...
        {k++;
         ...
         while (t>1)
          {t--;
           ...
           j=(topovecind[t])/lattice_size_x+1;
           if (i==lattice_size_x) j--;
		   if (mask[i][j]==1) calculatedischargefluxes(i,j);}
	     for (j=1;j<=lattice_size_y;j++)
          for (i=1;i<=lattice_size_x;i++)
		   if (mask[i][j]==1) 
