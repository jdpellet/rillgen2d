...
int main()
{
	 ...
	 fp3=fopen("./tau.txt","w");
	 fscanf(fpin,"%d %s\n",&flagforequation,temp);
	 fscanf(fpin,"%d %s\n",&flagfordynamicmode,temp);
	 fscanf(fpin,"%d %s\n",&flagformask,temp);
	 ...
	 if (flagfordynamicmode!=0)
	  {fpin=fopen("./dynamicinput.txt","r");
	   fscanf(fpin,"%d %s",&flagforrainrasters,temp);		 
	   fscanf(fpin,"%f %s",&rillerodibility,temp);		
	   fscanf(fpin,"%f %s",&bulkdensity,temp);		 	 
	   fscanf(fpin,"%f %s",&timescaleofinterest,temp);
       fscanf(fpin,"%f %s",&durationofdata,temp);
	   fscanf(fpin,"%f %s",&timestep,temp);
	   fscanf(fpin,"%d %s",&numberofraingages,temp);
