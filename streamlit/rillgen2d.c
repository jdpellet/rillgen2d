...
void mfdflowroute(i, j) long i, j;
{
    float tot;
        ...
        for (j = 1; j <= lattice_size_y; j++)
        {
            for (i = 1; i <= lattice_size_x; i++)
            {
                ...
                while (count > 0)
                {
                    pop();
                    max = topo[ic][jc];
                    if (topo[iup[ic]][jc] < max)
                        max = topo[iup[ic]][jc];
                    if (topo[idown[ic]][jc] < max)
                        max = topo[idown[ic]][jc];
                    if (topo[ic][jup[jc]] < max)
                        max = topo[ic][jup[jc]];
                    if (topo[ic][jdown[jc]] < max)
                        max = topo[ic][jdown[jc]];
                    if (topo[iup[ic]][jup[jc]] < max)
                        max = topo[iup[ic]][jup[jc]];
                    if (topo[idown[ic]][jdown[jc]] < max)
                        max = topo[idown[ic]][jdown[jc]];
                    if (topo[idown[ic]][jup[jc]] < max)
                        max = topo[idown[ic]][jup[jc]];
                    if (topo[iup[ic]][jdown[jc]] < max)
                        max = topo[iup[ic]][jdown[jc]];
...
int main()
{
    ...
    for (j = 1; j <= lattice_size_y; j++)
        for (i = 1; i <= lattice_size_x; i++)
        {
            ...
            if (mask[i][j] == 1)
                avsinofslope[i][j] = sin(atan(slope[i][j]));
            else
                avsinofslope[i][j] = 0;
        ...
        for (i = 1; i <= lattice_size_x; i++)
            if (mask[i][j] == 1)
            {
                ...
                if ((rockcover[i][j] < 0.99) && (rockcover[i][j] > 0.3))
                    taucarmor[i][j] /= exp(-4 * (rockcover[i][j] - 0.1));
    ...
    else
    {
        ...
        if (flagforrainrasters == 0)
        {
            ...
            for (m = 1; m <= numberofraingages; m++)
                fscanf(fpin, "%f %f", &raingagex[m], &raingagey[m]);
            for (j = 1; j <= lattice_size_y; j++)
                for (i = 1; i <= lattice_size_x; i++)
                {
                    mindist = large;
        ...
        if (flagforrainrasters == 0)
            ...
            fp5 = fopen("./rainrasters.txt", "r");
        dynamic = 1;
        for (n = 1; n <= numberofraindata; n++)
        {
            if (flagforrainrasters == 0)
                for (m = 1; m <= numberofraingages; m++)
                    fscanf(fpin, "%f", &rainvalues[m]);
            else
            {
                for (j = 1; j <= lattice_size_y; j++)
                    for (i = 1; i <= lattice_size_x; i++)
            ...
            for (j = 1; j <= lattice_size_y; j++)
                ...
                {
                    if (flagforrainrasters == 0)
                        rainl[i][j] = rainvalues[closestgage[i][j]];
                    if (rainl[i][j] > rain[i][j])
                        rain[i][j] = rainl[i][j];
        ...
        fclose(fpin);
        fclose(fp4);
        if (flagforrainrasters == 1)
            fclose(fp5);
    }
    for (j = 1; j <= lattice_size_y; j++)
        for (i = 1; i <= lattice_size_x; i++)
            if (slope[i][j] < tanangleofinternalfriction)
                f[i][j] = tauwithoutrain[i][j] * pow(rain[i][j], 0.666667) / taucarmor[i][j];
            else
                f[i][j] = 1;
    for (j = 1; j <= lattice_size_y; j++)
        for (i = 1; i <= lattice_size_x; i++)
        {
            if (mask[i][j] == 1)
            {
                if ((f[i][j] < 2) && (f[i][j] > -large) && (f[i][j] < large))
                    fprintf(fp1, "%f\n", f[i][j]);
                else
                {
                    if (f[i][j] > 2)
                ...
                fprintf(fp1, "0.0\n");
            if (mask[i][j] == 1)
                fprintf(fp3, "%f\n", tauwithoutrain[i][j] * pow(rain[i][j], 0.666667));
            else
                fprintf(fp3, "0.0\n");
        }
    ...
    for (j = 1; j <= lattice_size_y; j++)
        for (i = 1; i <= lattice_size_x; i++)
            ...
            else
            {
                if (f[i][j] > 1)
                    fprintf(fp2, "255 0 0\n");
                else if (f[i][j] > yellowthreshold)
                    fprintf(fp2, "255 255 0\n");
                else
                    fprintf(fp2, "255 255 255\n");
            }
    fclose(fp0);
    fclose(fp1);
    fclose(fp2);
